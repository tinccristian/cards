...
int GameScreen::drawPileViewer(const std::string& title,
    ...
    // X close button (top-right corner of panel)
    {
        ...
        int xw = MeasureText("X", closeFontSize);
        DrawText("X",
    ...
    // --- Grid layout: evenly spaced card slots with preserved aspect ratio ---
    ...
    const float cardHeightFromRows = ((float)gridH - gridGap * (visibleRows - 1)) / (float)visibleRows;
    ...
    int maxScroll = std::max(0, rows - fullyVisibleRows);
    ...
    for (int idx = 0; idx < total; ++idx) {
        int col = idx % cols;
        int row = idx / cols;
        float cx  = gridX + col * (drawWidth + gridGap);
        float cy  = (float)gridTop + (row - scrollOffset) * rowStride;

        if (cy + drawHeight < gridTop || cy > gridTop + gridH) continue; // out of view
...
RewardPopupAction GameScreen::drawRewardPopup(const RewardState& rewards,
    ...
    for (int index = 0; index < static_cast<int>(entries.size()); ++index) {
        ...
        const bool hovered = allowInteraction && mouseOver(rect);
        DrawRectangleRec(rect, hovered ? Colors::button_hover : Colors::button_bg);
        DrawRectangleLinesEx(rect, scalef(LayoutConfig::ThinBorderThickness), hovered ? entries[index].titleColor : Colors::card_border);
